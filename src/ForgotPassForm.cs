...
namespace Chess
{
    public partial class ForgotPassForm : Form
    {
        ...
        private string showPassPath = Application.StartupPath.Replace(@"bin\Debug", "resurse") + @"\ShowPass.png",
            hidePassPath = Application.StartupPath.Replace(@"bin\Debug", "resurse") + @"\HidePass.png",
            connStr = "Server=paul;Database=Chess;User Id=Paul;Password=Paul1234;";
        ...
        private void StyleButtons()
        {
            ...
            btnShowHide.MouseClick += (sender, e) =>
            {
                if (!ShowPass)
                {
                    btnShowHide.BackgroundImage = Image.FromFile(showPassPath);
                    ShowPass = true;
                    txtNewPass.PasswordChar = '\0';
                }
                else
                {
                    btnShowHide.BackgroundImage = Image.FromFile(hidePassPath);
                    ShowPass = false;
            ...
            btnShowHide1.MouseClick += (sender, e) =>
            {
                if (!ShowPass1)
                {
                    btnShowHide1.BackgroundImage = Image.FromFile(showPassPath);
                    ShowPass1 = true;
                    txtOldPass.PasswordChar = '\0';
                }
                else
                {
                    btnShowHide1.BackgroundImage = Image.FromFile(hidePassPath);
                    ShowPass1 = false;
